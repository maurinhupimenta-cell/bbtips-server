...
function apiRealResultsSeries(marketKey,liga){
  ...
  // janela movel de 20 jogos -> % do mercado, igual a logica nativa do Caramelo
  const used=rows.slice(-Math.max(120,Math.min(rows.length,360)));
  const win=20;
  const blockCount=Math.floor(used.length/win);
  if(blockCount<2)return null;
  const pct=[];
  for(let b=0;b<blockCount;b++){
    const block=used.slice(b*win,b*win+win);
    const g=block.filter(r=>apiMarketPays(r.score,wanted)).length;
    pct.push(Math.round(g/block.length*100));
  }
  // suaviza para a curva nao ficar em degraus de 5%
  const smooth=[];
