...
function apiRealResultsSeries(marketKey,liga){
  ...
  // pre-calcula "paga?" 1x por jogo
  const pays=new Array(used.length);
  for(let i=0;i<used.length;i++)pays[i]=apiMarketPays(used[i].score,wanted)?1:0;
  // janela deslizante INCREMENTAL (soma corrente, sem refazer filter)
  let running=0;
  for(let i=0;i<win;i++)running+=pays[i];
  const pct=[Math.round(running/win*100)];
  for(let i=win;i<used.length;i++){running+=pays[i]-pays[i-win];pct.push(Math.round(running/win*100));}
  // suavizacao leve
