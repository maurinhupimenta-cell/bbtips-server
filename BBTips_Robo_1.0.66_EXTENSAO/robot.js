...
function apiMarketPays(score,marketKey){
  ...
  const total=Number.isFinite(Number(score.t))?Number(score.t):a+b;
  if(marketKey==="over25")return total>2.5;
  if(marketKey==="over35")return total>3.5;
  if(marketKey==="over15")return total>1.5;
  if(marketKey==="under25")return total<2.5;
  if(marketKey==="under35")return total<3.5;
  if(marketKey==="ambas_sim")return a>0&&b>0;
  if(marketKey==="ambas_nao")return!(a>0&&b>0);
  return total>2.5;
}
function apiRealResultsSeries(marketKey,liga){
  const wanted=String(marketKey||CONFIG.market||"over25");
  const rows=API_ROWS
    .filter(r=>r&&r.score&&!r.future&&(!liga||!r.liga||Number(r.liga)===Number(liga)))
    .slice()
    .sort((x,y)=>resultAge(y)-resultAge(x)||(x.idx||0)-(y.idx||0));
  if(rows.length<30)return null;
  // Janela DESLIZANTE: para cada jogo, % do mercado nos ultimos 20 jogos.
  // Gera 1 ponto por jogo -> curva densa e suave, igual ao grafico do site.
  const win=20;
  const used=rows.slice(-Math.min(rows.length,420));
  if(used.length<win+10)return null;
  const pct=[];
  for(let i=win;i<=used.length;i++){
    const block=used.slice(i-win,i);
    const g=block.filter(r=>apiMarketPays(r.score,wanted)).length;
    pct.push(Math.round(g/win*100));
  }
  // leve suavizacao para tirar serrilhado
  const smooth=[];
  for(let i=0;i<pct.length;i++){
    const a=pct[Math.max(0,i-1)],c=pct[i],d=pct[Math.min(pct.length-1,i+1)];
    smooth.push(Math.round((a+c+d)/3));
  }
  return {values:smooth,count:rows.length,marketKey:wanted};
}
window.BBTipsRobo={
  analyze,
  config:CONFIG,
...
(function () {
  ...
  function loop() {
    ...
    const apiReal = (!nativeDataPresent) ? readApiRealGraphData() : null;
